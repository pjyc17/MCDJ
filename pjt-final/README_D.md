...
또, 태어난 년도에 정확히 carousel이 멈추도록 만들었다.(기존은 정확하지 않고 비슷하게 멈춤)
...
## 자랑
vue frontend도 이쁘게 꾸몄지만, 무엇보다 요청을 최대한 django의 orm으로 해결했기에, 응답속도가 빠르다.

쿼리문을 짜는데, 어려움이 있어, 교수님께 도움도 받으며, 복잡한 orm을 구현했다.

(annotate에 여러 요소를 쓰면, group by 문제로 원하는 값이 안나오기에, annotate 함수에 distinct를 써주면 해결된다.)
